resource "akamai_dns_record" "tf_demo_dns_records" {

  for_each = {
    for idx, challenge in akamai_cps_dv_enrollment.enrollment.dns_challenges :
    challenge.domain => challenge if challenge.response_body != null
  }

  zone       = "jaescalo.online"
  recordtype = "TXT"
  ttl        = "60"
  target     = [each.value.response_body]
  name       = each.value.full_path
  depends_on = [akamai_cps_dv_enrollment.enrollment]
}

# Wait for EdgeDNS zone changes to propagate (5 min may be excessive)
resource "time_sleep" "wait_for_dns_records" {
  create_duration = "300s"
  depends_on      = [akamai_dns_record.tf_demo_dns_records]
}
